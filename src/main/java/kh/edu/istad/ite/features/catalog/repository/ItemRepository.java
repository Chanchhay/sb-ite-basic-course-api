...
public interface ItemRepository extends JpaRepository<Item, UUID>, JpaSpecificationExecutor<Item> {
        ...
        @Query("""
            ...
            JOIN ItemChannel ic ON ic.item = i
            JOIN SalesChannel sc ON sc = ic.salesChannel
            WHERE i.business.id = :businessId
              AND i.status = :status
              AND sc.code = :channelCode
              AND ic.isEnabled = true
            ORDER BY i.name ASC
        """)
        List<Item> findActiveItemsByBusinessIdAndChannelCode(
                        @Param("businessId") UUID businessId,
                        @Param("status") ItemStatus status,
                        @Param("channelCode") String channelCode);
        ...
        @Query("""
            ...
            WHERE i.business.id = :businessId
              AND i.status = :status
              AND sc.code IN :channelCodes
              AND ic.isEnabled = true
            ORDER BY i.name ASC
        """)
        List<Item> findActiveItemsByBusinessIdAndChannelCodes(
                        @Param("businessId") UUID businessId,
                        @Param("status") ItemStatus status,
        ...
        @Query("""
            ...
            ORDER BY i.name ASC
        """)
        Page<Item> findActiveItemsByBusinessIdAndChannelCodes(
                        @Param("businessId") UUID businessId,
                        @Param("status") ItemStatus status,
                        @Param("channelCodes") List<String> channelCodes,
                        Pageable pageable);
        ...
        @Query("""
            ...
            WHERE i.business.id = :businessId
              AND i.status = :status
              AND i.itemGroup.id = :itemGroupId
              AND sc.code IN :channelCodes
              AND ic.isEnabled = true
            ORDER BY i.name ASC
        """)
        Page<Item> findActiveItemsByBusinessIdAndStatusAndItemGroup_IdAndChannelCodes(
                        @Param("businessId") UUID businessId,
                        @Param("status") ItemStatus status,
                        @Param("itemGroupId") UUID itemGroupId,
                        @Param("channelCodes") List<String> channelCodes,
        ...
        @Query("""
            ...
            ORDER BY i.name ASC
        """)
        Page<Item> findActiveItemsByBusinessIdAndStatusAndNameContainingAndChannelCodes(
                        @Param("businessId") UUID businessId,
                        @Param("status") ItemStatus status,
