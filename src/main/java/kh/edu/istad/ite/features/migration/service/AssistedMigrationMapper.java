...
public class AssistedMigrationMapper {
    ...
    public AssistedMigrationDtos.SourceResponse toResponse(AssistedMigrationSource source) {
        return new AssistedMigrationDtos.SourceResponse(
                source.getId(),
                source.getOrdinal(),
                source.isPrimary(),
                source.getPurpose(),
                ...
                source.getSourceColumns(),
                source.getColumnMappings(),
                source.isAnalyzed()
        );
    }

    public AssistedMigrationDtos.SourceAnalysisResponse toResponse(
            AssistedMigrationService.Analysis analysis
    ) {
        return new AssistedMigrationDtos.SourceAnalysisResponse(
                analysis.sourceId(),
                analysis.fileName(),
                analysis.purpose(),
                analysis.profile().rows(),
                analysis.profile().columns().stream().map(this::toResponse).toList(),
                analysis.suggestions().stream().map(this::toResponse).toList()
        );
    }

    /**
     * A suggested join, with what it would actually do already attached.
     *
     * The quality is not a separate call. An operator shown a confident
     * suggestion and asked to fetch the numbers separately will approve it
     * without fetching them.
     */
    public AssistedMigrationDtos.JoinSuggestionResponse toResponse(JoinSuggestion suggestion) {
        return new AssistedMigrationDtos.JoinSuggestionResponse(
                suggestion.leftSourceId(),
                suggestion.leftSourceName(),
                suggestion.leftColumn(),
                suggestion.rightSourceId(),
                suggestion.rightSourceName(),
                suggestion.rightColumn(),
                suggestion.confidence(),
                suggestion.reason(),
                new AssistedMigrationDtos.JoinQualityResponse(
                        null,
                        suggestion.leftSourceId(),
                        suggestion.leftSourceName(),
                        suggestion.leftColumn(),
                        ...
                        suggestion.rightColumn(),
                        null,
                        suggestion.quality().leftRows(),
                        suggestion.quality().rightRows(),
                        suggestion.quality().matchedLeftRows(),
                        suggestion.quality().unmatchedLeftRows(),
                        suggestion.quality().unmatchedRightRows(),
                        suggestion.quality().duplicateLeftKeys(),
                        suggestion.quality().duplicateRightKeys(),
                        suggestion.quality().cardinality(),
                        suggestion.quality().isUsable())
        );
    }

    public AssistedMigrationDtos.MissingFieldsResponse toResponse(MissingFieldReport report) {
        return new AssistedMigrationDtos.MissingFieldsResponse(
                report.fields().stream()
                        .map(status -> new AssistedMigrationDtos.MissingFieldResponse(
                                status.field().name(),
                                status.label(),
                                status.behaviour().name(),
                                status.filled(),
                                status.missing(),
                                status.blocking(),
                                status.suggestion(),
